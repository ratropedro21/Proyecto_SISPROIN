...
namespace SISPROIN.Clases
{
    ...
    public class CrearTicket
    {
        ...
        int maxCar = 40, cortar;//Para una impresora ticketera que imprime a 45 columnas. La variable cortar cortara el texto cuando rebase el limte.
        ...
        public void AgregaArtMovimientos(string articulo, decimal cant, int cant_und, decimal cost)
        {
            //Valida que cant precio e importe esten dentro del rango.
            if (cant.ToString().Length <= 5 && cant_und.ToString().Length <= 7 && cost.ToString().Length <= 8)
            {
                ...
                if (articulo.Length > 10)
                {
                    ...
                    nroEspacios = (7 - cant_und.ToString().Length);
                    espacios = "";
                    for (int i = 0; i < nroEspacios; i++)
                    {
                    ...
                    elemento += espacios + cant_und.ToString();//Agregamos el precio a la variable elemento

                    //Colocar el importe a la derecha.
                    nroEspacios = (8 - cost.ToString().Length);
                    espacios = "";
                    for (int i = 0; i < nroEspacios; i++)
                    {
                        espacios += " ";
                    }
                    elemento += espacios + cost.ToString();//Agregamos el importe alineado a la derecha
                ...
                else //Si no es mayor solo agregarlo, sin dar saltos de lineas
                {
                    ...
                    elemento += espacios + cant_und.ToString();
                    ...
                    elemento += espacios + cost.ToString();
        ...
        public void CortaTicket()
        {
            ...
            linea.AppendLine("\x1B" + "d" + "\x0"); //Avanza 9 renglones, Tambien varian
        }
        //Para abrir el cajon
        public void AbreCajon()
        {
            //Estos tambien varian, tienen que ver el manual de la impresora para poner los correctos.
            linea.AppendLine("\x1B" + "p" + "\x00" + "\x0F" + "\x80"); //Caracteres de apertura cajon 0
